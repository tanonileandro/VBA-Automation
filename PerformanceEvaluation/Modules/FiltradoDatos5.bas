Attribute VB_Name = "FiltradoDatos5"
Sub FiltrarDatos5()
    ...
    wsResultados.Range("Y5:AD" & wsResultados.Rows.count).ClearContents
    ...
    For Each evalRow In tblEvaluacion.ListRows
        If evalRow.Range(29).Value >= 16 Then
            data80Plus.Add Array(CStr(evalRow.Range(2).Value) & CStr(evalRow.Range(3).Value), evalRow.Range(29).Value)
        ElseIf evalRow.Range(29).Value > 12 And evalRow.Range(29).Value < 16 Then
            data60To80.Add Array(CStr(evalRow.Range(2).Value) & CStr(evalRow.Range(3).Value), evalRow.Range(29).Value)
        ElseIf evalRow.Range(29).Value <= 12 Then
            data60Minus.Add Array(CStr(evalRow.Range(2).Value) & CStr(evalRow.Range(3).Value), evalRow.Range(29).Value)
        End If
    Next evalRow
    ...
    For i = 1 To data80Plus.count
        wsResultados.Cells(5 + i - 1, 25).Value = data80Plus(i)(0)
        wsResultados.Cells(5 + i - 1, 26).Value = data80Plus(i)(1)
    Next i
    
    For i = 1 To data60To80.count
        wsResultados.Cells(5 + i - 1, 27).Value = data60To80(i)(0)
        wsResultados.Cells(5 + i - 1, 28).Value = data60To80(i)(1)
    Next i
    
    For i = 1 To data60Minus.count
        wsResultados.Cells(5 + i - 1, 29).Value = data60Minus(i)(0)
        wsResultados.Cells(5 + i - 1, 30).Value = data60Minus(i)(1)
    Next i
