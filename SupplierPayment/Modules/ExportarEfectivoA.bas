Attribute VB_Name = "ExportarEfectivoA"
Sub ExportarEfectivoA()
    ...
    Set wsDestino = ThisWorkbook.Sheets("EFECTIVO A")
    On Error GoTo 0
    
    If wsDestino Is Nothing Then
        Sheets.Add(After:=Sheets(Sheets.Count)).Name = "EFECTIVO A"
        Set wsDestino = Sheets("EFECTIVO A")
    End If
    ...
    On Error Resume Next
    Set tblDestino = wsDestino.ListObjects("Tabla8")
    On Error GoTo 0
    ...
    For i = 3 To ultimaFilaProveedores
        If Not IsEmpty(wsProveedores.Cells(i, 3).Value) And Not IsEmpty(wsProveedores.Cells(i, 11).Value) Then
            If UCase(wsProveedores.Cells(i, 3).Value) = "A" And UCase(wsProveedores.Cells(i, 11).Value) = "EFECTIVO" Then
    ...
    MsgBox "Los datos se exportaron correctamente a la hoja 'EFECTIVO A'.", vbInformation
End Sub
