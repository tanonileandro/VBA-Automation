Attribute VB_Name = "ProcesarChequesA"
Sub ProcesarChequesA()
    ...
    hojaMensual.Range("E23").Value = Abs(sumaValores)
...
Function SumarValoresCheques(hoja As Worksheet) As Double
    ...
    For fila = 2 To hoja.Cells(hoja.Rows.Count, "K").End(xlUp).Row
        If Not IsError(hoja.Cells(fila, 11).Value) Then
            ...
            If tipoPago = "cheques" And UCase(hoja.Cells(fila, 3).Value) Like "*A*" Then
