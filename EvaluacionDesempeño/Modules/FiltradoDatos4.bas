Attribute VB_Name = "FiltradoDatos4"
Sub FiltrarDatos4()
    ...
    wsResultados.Range("S5:X" & wsResultados.Rows.count).ClearContents
    ...
    For Each evalRow In tblEvaluacion.ListRows
        If evalRow.Range(27).Value >= 16 Then
            data80Plus.Add Array(CStr(evalRow.Range(2).Value) & CStr(evalRow.Range(3).Value), evalRow.Range(27).Value)
        ElseIf evalRow.Range(27).Value > 12 And evalRow.Range(27).Value < 16 Then
            data60To80.Add Array(CStr(evalRow.Range(2).Value) & CStr(evalRow.Range(3).Value), evalRow.Range(27).Value)
        ElseIf evalRow.Range(27).Value <= 12 Then
            data60Minus.Add Array(CStr(evalRow.Range(2).Value) & CStr(evalRow.Range(3).Value), evalRow.Range(27).Value)
        End If
    Next evalRow
    ...
    For i = 1 To data80Plus.count
        wsResultados.Cells(5 + i - 1, 19).Value = data80Plus(i)(0)
        wsResultados.Cells(5 + i - 1, 20).Value = data80Plus(i)(1)
    Next i
    
    For i = 1 To data60To80.count
        wsResultados.Cells(5 + i - 1, 21).Value = data60To80(i)(0)
        wsResultados.Cells(5 + i - 1, 22).Value = data60To80(i)(1)
    Next i
    
    For i = 1 To data60Minus.count
        wsResultados.Cells(5 + i - 1, 23).Value = data60Minus(i)(0)
        wsResultados.Cells(5 + i - 1, 24).Value = data60Minus(i)(1)
    Next i
