Attribute VB_Name = "FiltradoDatos"
Sub FiltrarDatos1()
    ...
    wsResultados.Range("A5:F" & wsResultados.Rows.count).ClearContents
    ...
    For Each evalRow In tblEvaluacion.ListRows
        If evalRow.Range(25).Value >= 80 Then
            data80Plus.Add Array(CStr(evalRow.Range(2).Value) & CStr(evalRow.Range(3).Value), evalRow.Range(25).Value)
        ElseIf evalRow.Range(25).Value > 60 And evalRow.Range(25).Value < 80 Then
            data60To80.Add Array(CStr(evalRow.Range(2).Value) & CStr(evalRow.Range(3).Value), evalRow.Range(25).Value)
        ElseIf evalRow.Range(25).Value <= 60 Then
            data60Minus.Add Array(CStr(evalRow.Range(2).Value) & CStr(evalRow.Range(3).Value), evalRow.Range(25).Value)
        End If
    Next evalRow
    ...
    For i = 1 To data80Plus.count
        wsResultados.Cells(5 + i - 1, 1).Value = data80Plus(i)(0)
        wsResultados.Cells(5 + i - 1, 2).Value = data80Plus(i)(1)
    Next i
    
    For i = 1 To data60To80.count
        wsResultados.Cells(5 + i - 1, 3).Value = data60To80(i)(0)
        wsResultados.Cells(5 + i - 1, 4).Value = data60To80(i)(1)
    Next i
    
    For i = 1 To data60Minus.count
        wsResultados.Cells(5 + i - 1, 5).Value = data60Minus(i)(0)
        wsResultados.Cells(5 + i - 1, 6).Value = data60Minus(i)(1)
    Next i
