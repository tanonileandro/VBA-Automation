Attribute VB_Name = "FiltradoDatos3"
Sub FiltrarDatos3()
    ...
    wsResultados.Range("M5:R" & wsResultados.Rows.count).ClearContents
    ...
    For Each evalRow In tblEvaluacion.ListRows
        If evalRow.Range(26).Value >= 16 Then
            data80Plus.Add Array(CStr(evalRow.Range(2).Value) & CStr(evalRow.Range(3).Value), evalRow.Range(26).Value)
        ElseIf evalRow.Range(26).Value > 12 And evalRow.Range(26).Value < 16 Then
            data60To80.Add Array(CStr(evalRow.Range(2).Value) & CStr(evalRow.Range(3).Value), evalRow.Range(26).Value)
        ElseIf evalRow.Range(26).Value <= 12 Then
            data60Minus.Add Array(CStr(evalRow.Range(2).Value) & CStr(evalRow.Range(3).Value), evalRow.Range(26).Value)
        End If
    Next evalRow
    ...
    For i = 1 To data80Plus.count
        wsResultados.Cells(5 + i - 1, 13).Value = data80Plus(i)(0)
        wsResultados.Cells(5 + i - 1, 14).Value = data80Plus(i)(1)
    Next i
    
    For i = 1 To data60To80.count
        wsResultados.Cells(5 + i - 1, 15).Value = data60To80(i)(0)
        wsResultados.Cells(5 + i - 1, 16).Value = data60To80(i)(1)
    Next i
    
    For i = 1 To data60Minus.count
        wsResultados.Cells(5 + i - 1, 17).Value = data60Minus(i)(0)
        wsResultados.Cells(5 + i - 1, 18).Value = data60Minus(i)(1)
    Next i
