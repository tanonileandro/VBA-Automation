Attribute VB_Name = "ExportarTransferenciaA"
Sub ExportarTransferenciaA()
    ...
    Set wsDestino = ThisWorkbook.Sheets("TRANSF. A")
    On Error GoTo 0
    
    If wsDestino Is Nothing Then
        Sheets.Add(After:=Sheets(Sheets.Count)).Name = "TRANSF. A"
        Set wsDestino = Sheets("TRANSF. A")
    End If
    ...
    On Error Resume Next
    Set tblDestino = wsDestino.ListObjects("Tabla6")
    On Error GoTo 0
    ...
    For i = 3 To ultimaFilaProveedores
        If Not IsEmpty(wsProveedores.Cells(i, 3).Value) And Not IsEmpty(wsProveedores.Cells(i, 11).Value) Then
            If UCase(wsProveedores.Cells(i, 3).Value) = "A" And UCase(wsProveedores.Cells(i, 11).Value) = "TRANSFERENCIA" Then
    ...
    MsgBox "Los datos se exportaron correctamente a la hoja 'TRANS. A'.", vbInformation
End Sub
